...
const ObraDetail = () => {
  ...
  const hasGallery = obra.gallery && obra.gallery.length > 0;
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="pt-20">
        <div className="relative bg-gradient-to-br from-terracotta-dark to-warm-brown overflow-hidden rounded-b-3xl mx-4 sm:mx-6">
          {obra.coverImage && (
            <img src={obra.coverImage} alt={obra.title} className="absolute inset-0 w-full h-full object-cover opacity-30" />
          )}
      ...
      {hasGallery && (
        <section className="container mx-auto px-6 -mt-6 relative z-10">
          <ImageCarousel images={obra.gallery!} />
        </section>
      )}
          ...
          <div className="lg:col-span-3 space-y-8">
            <div className="bg-cream rounded-2xl p-8">
              <div className="flex items-center gap-3 mb-4">
                {Icon && <Icon className="w-5 h-5 text-coral" />}
                <h2 className="font-serif text-2xl font-bold text-warm-brown">Sinopsis</h2>
              </div>
              <p className="text-warm-brown/70 leading-relaxed text-base">{obra.synopsis}</p>
            </div>
            <div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-sage-light rounded-2xl p-6 space-y-5">
              ...
              <div className="flex items-start gap-3">
                ...
                <div><p className="text-xs text-warm-brown/50">Fecha</p><p className="font-semibold text-warm-brown">{obra.date}</p></div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div><p className="text-xs text-warm-brown/50">Hora</p><p className="font-semibold text-warm-brown">{obra.time}</p></div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div><p className="text-xs text-warm-brown/50">Lugar</p><p className="font-semibold text-warm-brown">{obra.venue}</p><p className="text-xs text-warm-brown/50">{obra.venueAddress}</p></div>
              </div>
              <div className="flex items-start gap-3">
